...
> **Important distinction:** C1/C2/C4 (cartouche/oval) can be explored with AI generation. **C3 (geometric shared-stroke monogram / Loewe register) cannot reliably be generated by AI** — the AI will produce swirling loops instead of the geometric construction you want. Run C3 in Figma instead using the construction workflow in `projects/alt-cosmetics-mark-workshop.md`. The C3 MJ prompts below are provided for loose form inspiration only — do not expect them to directly produce a usable geometric mark.
...
**C3 — Geometric shared-stroke monogram (Loewe register)**

⚠️ *Use these in Figma as loose form inspiration only. AI image generators will produce swirling loops, not the geometric construction you want. Build this mark using the Figma construction method in the workshop doc.*
```
abstract geometric brand mark, three letterforms unified as single flat shape, grid-based construction, shared stroke edges, consistent single line weight, architectural, luxury beauty brand, flat black on white, Bauhaus lineage --ar 1:1 --style raw --v 6.1 --no calligraphy, swash, organic curves, loops, gradients, script
```
```
geometric monogram, letters as pure flat geometric forms, shared stroke construction, consistent weight, negative space within form, abstract enough to not read immediately, luxury, minimal, black on white, flat 2D --ar 1:1 --style raw --v 6.1 --no calligraphy, loops, script, ornament, gradients
```
...
**To push Direction C cartouche further:** add `art deco medallion` or `constructivist emblem` or `architectural oval seal`
**To push Direction C geometric (for loose form inspiration):** add `Bauhaus monogram` or `De Stijl letterform` or `constructivist flat mark` — but remember, take the overall form to Figma and construct from scratch
